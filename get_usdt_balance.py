...
def get_trc20token_balanceOf_from_trongridV1(address):
    ...
    usdt_info_trongrid = requests.get(trongrid_url+address,headers=headers,timeout=5).json() # 获取address的余额信息
...
def get_trc20token_info_from_tronscan(tronscan_url,excel_path):
    ...
    usdt_info_form_tronscan_content = requests.get(tronscan_url,headers=headers,timeout=5).json() # 获取tronscan 上USDT部分holder的余额信息
    print(usdt_info_form_tronscan_content)  
