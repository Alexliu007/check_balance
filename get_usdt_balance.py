...
# 通过triggerconstantcontract查询余额       
def get_balance_from_trig(address,contract_address):
    address_hex = tron.address.to_hex(address)
    parameter = "0000000000000000000000"+ str(address_hex)
    trans_payload = {
    ...
    trans_url = 'https://api.trongrid.io/wallet/triggerconstantcontract'
    trans_respon = requests.post(trans_url,json=trans_payload,timeout=20)
    balance = int(trans_respon.json()['constant_result'][0],16)
    return balance
...
def main():
    ...
    # 初始化excel表
    #--------------
    path = "/Users/tron/Desktop/usdt_check/usdt_check_0324.xls" # excel表路径
    sheet_name = "usdt_balance_diff_check" 
    value_title = [["address", "tronscan_balance", "trongrid_balance","trigger_balance","check_time"],]
